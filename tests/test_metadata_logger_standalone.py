...
def test_pmrda_metadata_logger_standalone_workflow():
    ...
    # 2. Log User Prompt
    user_msg_id = logger.log_user_message(session_id, "Show total applications count")
    assert user_msg_id is not None

    # 3. Log Query Execution
    query_log_id = logger.log_query_execution(
        query_text="SELECT COUNT(*) FROM rts_applications;",
        status="SUCCESS",
        execution_time_ms=12.4,
        result_row_count=1500,
    )
    ...
    agent_msg_id = logger.log_agent_message(
        ...
        content="Total applications till now: 1,500",
        sql_used="SELECT COUNT(*) FROM rts_applications;",
        execution_time_ms=12.4,
        total_records=1500,
    )
    ...
    unmatched_id = logger.log_unmatched_query(
        ...
        reason="OFF_TOPIC_PMRDA_SCOPE_REJECTION",
        session_id=session_id,
    )
    ...
    assert history[0]["content"] == "Show total applications count"
    assert history[1]["sender"] == "agent"
    assert history[1]["sql_used"] == "SELECT COUNT(*) FROM rts_applications;"

    # 7. Fetch Developer Info
    dev_info_list = logger.fetch_developer_info(session_id)
    assert len(dev_info_list) == 1
    assert dev_info_list[0]["sql_used"] == "SELECT COUNT(*) FROM rts_applications;"
